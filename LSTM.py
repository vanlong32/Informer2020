...
root_path = './ETDataset/ETT-small/BTC/' # root path of data file
data_path = 'Binance_BTCUSDT_Full_5col_date_open_high_low_close.csv'

# load the dataset
dataset = pd.read_csv(root_path+data_path, header = 0,  engine='python') 

#dataset = dataframe.values


mydata = dataset[['open','high','low','close']].values

print(mydata)
print('hình dạng mảng: ', mydata.shape)



#cols_name = dataset.columns['date_open_high_low_close']

# lọc loại bỏ cột date
#dataset = dataset[cols_name]
...
scale_mydata = scaler.fit_transform(mydata)
